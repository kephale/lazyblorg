...
COMMENT_EMAIL_ADDRESS = 'computational.life@kyleharrington.com'
...
IGNORE_FOR_TOP_TAGS = ['personal']
IGNORE_FOR_TAG_CLOUD = ['personal', 'hardware', 'software']
...
ID_OF_ABOUT_PAGE = '2021-05-24-about'
...
##         as og:image)
TWITTER_IMAGE = 'https://kyleharrington.com/images/computational_life_logo_200x200.png'
...
WARN_IF_IMAGE_FILE_NOT_TAGGED_WITH = "computational_life"
...
import re
assert (re.match(r'(\+|-)[01][0123456789]:[012345][0123456789]',
                 TIME_ZONE_ADDON))
...
for currentdir in DIRECTORIES_WITH_IMAGE_ORIGINALS:
    if not os.path.isabs(currentdir):
        DIRECTORIES_WITH_IMAGE_ORIGINALS[index] = os.path.join(
            os.getcwd(), currentdir)
    if os.path.isdir(DIRECTORIES_WITH_IMAGE_ORIGINALS[index]):
        dir_set_and_found = True
    else:
        print('Warning: DIRECTORIES_WITH_IMAGE_ORIGINALS[' + str(index) +
              '] which is set to \"' + str(currentdir) +
...
if len(CUSTOMIZED_IMAGE_LINK_KEY) > 0:
    ...
    file_set_and_found = len(
        MEMACS_FILE_WITH_IMAGE_FILE_INDEX) > 1 and os.path.isfile(
            MEMACS_FILE_WITH_IMAGE_FILE_INDEX)
    file_set_and_not_found = len(
        MEMACS_FILE_WITH_IMAGE_FILE_INDEX) > 1 and not os.path.isfile(
            MEMACS_FILE_WITH_IMAGE_FILE_INDEX)
    dir_not_set = len(DIRECTORIES_WITH_IMAGE_ORIGINALS) < 1
    dir_set_and_not_found = len(
        DIRECTORIES_WITH_IMAGE_ORIGINALS) > 1 and not dir_set_and_found
